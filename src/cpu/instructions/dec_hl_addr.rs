...
pub struct DecHlAddr {
    counter: u8,
    value_register: Register8
}

impl Instruction for DecHlAddr {

    #[inline]
    fn from_opcode(opcode: &u8) -> Option<Box<dyn Instruction>> {
        if *opcode == 0x35 {
            return Some(Box::new(DecHlAddr { counter: 2, value_register: Register8::new(0x00) }));
        }
        None
    }
    ...
    fn act(&mut self, registers: &mut Registers, alu: &mut ALU, memory_controller: Arc<Mutex<MemoryController>>, _enable_interrupts: &mut bool, _is_halted: &mut bool) -> bool {
        if self.counter == 2 {
            ...
            alu.sub_no_carry(&mut self.value_register, &Register8::one());
        }
...
mod tests {
    ...
    reusable_testing_macro!(0x35, DecHlAddr);
    ...
    fn get_value_on_tick_2() {
        ...
        memory.lock().set(0xC000, 0x35);
        
        let mut instruction = DecHlAddr { counter: 2, value_register: Register8::new(0x00) };
    ...
    fn inc_value_on_tick_1() {
        ...
        let mut instruction = DecHlAddr { counter: 1, value_register: Register8::new(0x35) };
    ...
    fn get_next_instruction_on_tick_0() {
        ...
        let mut instruction = DecHlAddr { counter: 0, value_register: Register8::new(0x35) };
