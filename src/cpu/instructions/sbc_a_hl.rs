...
impl Instruction for SbcAHl {

    #[inline]
    fn from_opcode(opcode: &u8) -> Option<Box<dyn Instruction>> {
        if *opcode == 0x9E {
            return Some(Box::new(SbcAHl {
                counter: 1,
                value: 1
            }))
        }
    ...
    fn act(&mut self, registers: &mut Registers, alu: &mut ALU, memory_controller: Arc<Mutex<MemoryController>>, _enable_interrupts: &mut bool, _is_halted: &mut bool) -> bool {
        if self.counter == 1 {
            ...
            alu.sbc(&mut *registers.a.clone().borrow_mut(), &Register8::new(self.value));
            return true;
        }
...
mod add_a_a {
    ...
    reusable_testing_macro!(0x9E, SbcAHl);

    #[test]
    fn get_value_on_tick_1() {
        ...
        let mut instruction = SbcAHl { counter: 1, value:0 };
    ...
    #[test]
    fn sbc_a_from_n_on_tick_0_when_no_carry() {
        ...
        let mut instruction = SbcAHl { counter: 0, value: 0x12 };
    ...
    #[test]
    fn sbc_a_from_n_on_tick_0_when_carry() {
        let mut registers = Registers::new(0x3600, 0, 0, 0, 0xC001, 0);
        let memory = Arc::new(Mutex::new(MemoryController::new()));
        let mut alu = ALU::new(registers.f.clone());
        ...
        assert_eq!(true, result);
        assert_eq!(0x23, registers.a.borrow().get_value());
    }
}
