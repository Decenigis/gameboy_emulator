...
impl Instruction for AdcAHl {

    #[inline]
    fn from_opcode(opcode: &u8) -> Option<Box<dyn Instruction>> {
        if *opcode == 0x8E {
            return Some(Box::new(AdcAHl {
                counter: 1,
                value: 1
            }))
        }
    ...
    fn act(&mut self, registers: &mut Registers, alu: &mut ALU, memory_controller: Arc<Mutex<MemoryController>>, _enable_interrupts: &mut bool, _is_halted: &mut bool) -> bool {
        if self.counter == 1 {
            ...
            alu.adc(&mut *registers.a.clone().borrow_mut(), &Register8::new(self.value));
            return true;
        }
...
mod adc_a_a {
    ...
    reusable_testing_macro!(0x8E, AdcAHl);
    ...
    fn sub_a_from_n_on_tick_1() {
        ...
        let mut instruction = AdcAHl { counter: 1, value:0 };
    ...
    fn sub_a_from_n_on_tick_0() {
        ...
        let mut instruction = AdcAHl { counter: 0, value: 0x12 };
