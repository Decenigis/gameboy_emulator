...
impl Instruction for SwapHl {
    #[inline]
    fn from_opcode(opcode: &u8) -> Option<Box<dyn Instruction>> {
        if *opcode == 0x36 {
            return Some(Box::new(SwapHl{ counter: 3, value_register: Register8::new(0) }))
        }
        None
    }
...
mod tests {
    ...
    reusable_testing_macro!(0x36, SwapHl);
    ...
    fn gets_byte_on_tick_3() {
        ...
        let mut instruction = SwapHl { counter: 3, value_register: Register8::new(0) };
    ...
    #[test]
    fn swaps_when_nonzero_on_tick_2() {
        let mut registers = Registers::new(0, 0, 0, 0, 0xC000, 0);
        let memory = Arc::new(Mutex::new(MemoryController::new()));
        let mut alu = ALU::new(registers.f.clone());

        let mut instruction = SwapHl { counter: 2, value_register: Register8::new(0x12) };
    ...
    #[test]
    fn swaps_when_zero_on_tick_2() {
        let mut registers = Registers::new(0, 0, 0, 0, 0xC000, 0);
        let memory = Arc::new(Mutex::new(MemoryController::new()));
        let mut alu = ALU::new(registers.f.clone());

        let mut instruction = SwapHl { counter: 2, value_register: Register8::new(0x00) };
    ...
    fn write_value_on_tick_1() {
        ...
        let mut instruction = SwapHl { counter: 1, value_register: Register8::new(0x12) };
    ...
    fn get_next_instruction_on_tick_0() {
        ...
        let mut instruction = SwapHl { counter: 0, value_register: Register8::new(0x12) };
