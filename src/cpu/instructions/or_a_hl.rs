...
impl Instruction for OrAHl {

    #[inline]
    fn from_opcode(opcode: &u8) -> Option<Box<dyn Instruction>> {
        if *opcode == 0xB6 {
            return Some(Box::new(OrAHl {
                counter: 1,
                value: 1
            }))
        }
    ...
    fn act(&mut self, registers: &mut Registers, alu: &mut ALU, memory_controller: Arc<Mutex<MemoryController>>, _enable_interrupts: &mut bool, _is_halted: &mut bool) -> bool {
        if self.counter == 1 {
            ...
            alu.or(&mut *registers.a.borrow_mut(), &Register8::new(self.value));
            return true;
        }
...
mod tests {
    ...
    reusable_testing_macro!(0xB6, OrAHl);
    ...
    fn get_hl_on_tick_1() {
        ...
        let mut instruction = OrAHl { counter: 1, value:0 };
    ...
    #[test]
    fn or_values_on_tick_0() {
        let mut registers = Registers::new(0x2400, 0, 0, 0, 0xC001, 0);
        let memory = Arc::new(Mutex::new(MemoryController::new()));
        let mut alu = ALU::new(registers.f.clone());

        let mut instruction = OrAHl { counter: 0, value: 0x12 };
