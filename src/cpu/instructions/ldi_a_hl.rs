...
impl Instruction for LdiAHl {

    #[inline]
    fn from_opcode(opcode: &u8) -> Option<Box<dyn Instruction>> {
        if *opcode == 0x2A {
            return Some(Box::new(LdiAHl { counter: 1 }))
        }
        None
    }
...
mod tests {
    ...
    reusable_testing_macro!(0x2A, LdiAHl);
    ...
    fn load_n_on_tick_1() {
        ...
        let mut instruction = LdiAHl { counter: 1 };
    ...
    fn write_value_on_tick_0_and_get_next_instruction() {
        ...
        let mut instruction = LdiAHl { counter: 0 };
